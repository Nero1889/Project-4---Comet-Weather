...
SEARCH_BTN.addEventListener("click", () => SEARCH_OVERLAY.classList.add("open"));
...
const MOBILE_SEARCH_BAR = document.querySelector(".search-bar.expanded");
...
function displaySuggestions(suggestions, container, inputElement) {
    ...
    if (suggestions && suggestions.length > 0) {
        ...
        suggestions.forEach(suggestion => {
            ...
            TEXT_CONTAINER.style.marginLeft = "1rem";

            const CITY_TEXT = document.createElement("span");
            CITY_TEXT.textContent = suggestion.name;
            CITY_TEXT.style.color = "var(--white)";
            CITY_TEXT.style.fontSize = "var(--size-base)";

            const COUNTRY_TEXT = document.createElement("span");
            COUNTRY_TEXT.style.color = "var(--txt)";
            COUNTRY_TEXT.style.fontSize = "var(--size-xs)";
            COUNTRY_TEXT.style.marginTop = ".025rem";
...
function handleSuggestionSelection(suggestion, container, displayText) {
    ...
    if (DESKTOP_SEARCH_BAR && container === SUGGESTIONS_DROPDOWN) {
        DESKTOP_SEARCH_BAR.value = displayText;
        DESKTOP_SEARCH_BAR.classList.remove("suggestions-active");
    } else if (MOBILE_SEARCH_BAR && container === SUGGESTIONS_CONTAINER) {
        MOBILE_SEARCH_BAR.value = displayText;
        MOBILE_SEARCH_BAR.classList.remove("suggestions-active");
    }
}
...
if (DESKTOP_SEARCH_BAR && SUGGESTIONS_DROPDOWN) {
    DESKTOP_SEARCH_BAR.addEventListener("input", (e) => {
        fetchSuggestions(e.target.value, SUGGESTIONS_DROPDOWN, DESKTOP_SEARCH_BAR);
    });

    DESKTOP_SEARCH_BAR.addEventListener("focus", (e) => {
        if (e.target.value.trim()) {
            fetchSuggestions(e.target.value, SUGGESTIONS_DROPDOWN, DESKTOP_SEARCH_BAR);
        }
    });

    DESKTOP_SEARCH_BAR.addEventListener("blur", () => {
        setTimeout(() => {
    ...
    handleSearchInputEnter(DESKTOP_SEARCH_BAR, SUGGESTIONS_DROPDOWN);
}

if (MOBILE_SEARCH_BAR && SUGGESTIONS_CONTAINER) {
    MOBILE_SEARCH_BAR.addEventListener("input", (e) => {
        fetchSuggestions(e.target.value, SUGGESTIONS_CONTAINER, MOBILE_SEARCH_BAR);
    });

    MOBILE_SEARCH_BAR.addEventListener("focus", (e) => {
        if (e.target.value.trim()) {
            fetchSuggestions(e.target.value, SUGGESTIONS_CONTAINER, MOBILE_SEARCH_BAR);
        }
    });

    handleSearchInputEnter(MOBILE_SEARCH_BAR, SUGGESTIONS_CONTAINER);
}

/* Update Search Bar Roundness */
function toggleDropdown(active) {
    if (active) {
        DESKTOP_SEARCH_BAR.classList.add("open");
        SUGGESTIONS_DROPDOWN.style.display = "block";
    } else {
        DESKTOP_SEARCH_BAR.classList.remove("open");
        SUGGESTIONS_DROPDOWN.style.display = "none";
    }
}
...
DESKTOP_SEARCH_BAR.addEventListener("focus", (e) => {
    if (e.target.value.trim() !== "") toggleDropdown(true);
});
