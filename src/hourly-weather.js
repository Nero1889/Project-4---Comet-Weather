...
async function fetchHourlyForecast(city) {
    ...
    const API_URL = `https://api.openweathermap.org/data/2.5/forecast?q=${city}&appid=${API_KEY}&units=${UNITS_PARAM}`;
...
function isDaytime(timestamp, sunrise, sunset, timezoneOffset) {
    ...
    const SUNRISE_LOCAL_MS = (sunrise + timezoneOffset) * 1000;
    const SUNSET_LOCAL_MS = (sunset + timezoneOffset) * 1000;
    
    return LOCAL_TIME >= SUNRISE_LOCAL_MS && LOCAL_TIME < SUNSET_LOCAL_MS;
}
...
function displayHourlyForecast(forecastData, sunriseTimestamp, sunsetTimestamp, timezoneOffsetInSeconds) {
    ...
    const CURRENT_CITY_TIMESTAMP_HOUR_START = new Date(CITY_NOW_LOCAL_TIME.getFullYear(), CITY_NOW_LOCAL_TIME.getMonth(), CITY_NOW_LOCAL_TIME.getDate(), CITY_NOW_LOCAL_TIME.getHours()).getTime();
    ...
    forecastData.list.forEach(item => {
        ...
        if (FORECAST_LOCAL_TIMESTAMP >= CURRENT_CITY_TIMESTAMP_HOUR_START && displayedForecasts < HOURLY_DIVS.length) {
            ...
            if (ICON_ELEMENT) ICON_ELEMENT.src = `icons/${iconCode}`;
            if (TEMP_ELEMENT) TEMP_ELEMENT.textContent = `${TEMPERATURE}${UNIT_SYMBOL}`;
...
async function updateHourlyForecastForCity(city) {
    try {
        const MAIN_WEATHER_DATA = await fetchWeatherData(city); 
        const SUNRISE = MAIN_WEATHER_DATA.sys.sunrise;
        const SUNSET = MAIN_WEATHER_DATA.sys.sunset;
        const TIMEZONE = MAIN_WEATHER_DATA.timezone;

        const FORECAST_DATA = await fetchHourlyForecast(city);
        
        displayHourlyForecast(FORECAST_DATA, SUNRISE, SUNSET, TIMEZONE);
